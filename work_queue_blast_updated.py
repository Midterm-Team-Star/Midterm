...
if __name__ == '__main__':
  ...
  while (time.time() - start_time) < max_time: #Makes the program listen for a certain amount of time
    blast_path = "/usr/bin/blastx"
    if not os.path.exists(blast_path):
      blast_path = "~/ncbi-blast-2.9.0+/bin/blastx"
      if not os.path.exists(blast_path):
  ...
  # We create and dispatch a task for each filename given in the argument list
    for i in range(1, len(sys.argv)):
      ...
      # are using is the one being sent to the workers.
        command = "./blastx -query < %s > -out %s" % (infile, outfile)
      ...
      # following line, more often than not these are the same.
        t.specify_file(blast_path, "blastx", WORK_QUEUE_INPUT, cache=True)
